...
pub fn get_audio_cache(app_handle: &tauri::AppHandle, resource_name: &String) -> anyhow::Result<std::fs::File> {
...
pub fn put_audio_cache(app_handle: &tauri::AppHandle, resource_name: &String, bytes: &[u8]) -> anyhow::Result<()> {
	let audio_resource_path = get_audio_cache_dir(app_handle).unwrap().join(format!("{}.ogg", resource_name));
...
pub fn delete_audio_cache(app_handle: &tauri::AppHandle, resource_name: &String) -> anyhow::Result<()> {
	let audio_resource_path = get_audio_cache_dir(app_handle).unwrap().join(format!("{}.ogg", resource_name));
