 ...
 */
const getVehicleWithCapacityIdx = (
...
export const ActiveOccurrenceWizard: FunctionComponent<ActiveOccurrenceWizardProps> = ({ internalId, onClose }) => {
  ...
  const onVehiclesNext = useCallback(
    (vehicleIds: string[]) => {
  ...
  const onStaffNext = useCallback(
    (vehicleId: string, staff: string[]) => {
  ...
  const selectedThusFar = new Array(vehicleIdx)
    .fill(null)
    .map((_, idx) => vehicleAssignmentMap[vehicleIds[idx]])
    .flat()
  ...
  return (
    <FullscreenOverlay className="flex flex-col justify-center items-center">
      ...
      <div className="flex bg-[#000] rounded-xl z-10 w-full max-w-7xl max-h-full p-5 pb-10">
        {!internalId && activeSection === Section.Information && (
          <InformationPanel initialValue={ocurrenceInfo} onCancel={onClose} onNext={onInformationNext} />
        )}

        {!internalId && activeSection === Section.Occurrence && (
          <PickOccurrence
        ...
        {activeSection === Section.Vehicles && (
          <PickVehicles
